...
If you want to create a new Spring Boot starter project as you can do using the site `start.spring.io`, simply use the `project-new` command where you specify `spring-boot` as project type.
The command will issue a REST call against `start.spring.io`, will get a zip file containing the maven project created according to the parameters passed. The content of this file will be unzipped
locally according under the directory as defined by the `--named` parameter. When no Spring Boot starters / dependencies is defined, then `start.spring.io` will return a project containing
...
You can specify which Spring Boot starter version you want to use using the `--spring-boot-version` parameter like also the starters with
the `--dependencies` option, with is a defined as a space-separated list of dependencies.
+
----
project-new --named example --type spring-boot --spring-boot-version 1.5.3 --dependencies actuator web
----
+
Note that pressing tab after typing `--dependencies` option will result in Forge retrieving the list of
